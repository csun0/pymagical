...
for i=1:20 % editing for Rhesus Macaque
...
%load scATACseq data
fprintf('Loading scATACseq data ...\n\n')
...
[scATAC_cells.cell_index, scATAC_cells.cell_barcode, scATAC_cells.cell_type, scATAC_cells.subject_ID, scATAC_cells.condition]=...
    textread(scATAC_cellmeta_file_path, '%d %s %s %s %s', 'delimiter', '\t');
...
% scRNA_read_count_matrix=""
% scATAC_read_count_matrix=""


%load TF motif prior
fprintf('Loading TF motif prior ...\n\n')
...
%************************* B and L Matrix Output ***********************
fprintf('Writing final B and L probability matrices to files...\n');

% 1. Safely convert to character vector to avoid cell/string array issues
OutPath_char = char(Output_file_path); 
[filepath, name, ext] = fileparts(OutPath_char);

% 2. Use sprintf for bulletproof string building
B_matrix_file = fullfile(filepath, sprintf('%s_B_matrix.txt', name));
L_matrix_file = fullfile(filepath, sprintf('%s_L_matrix.txt', name));

% Write the B Matrix (TF-Peak Binding Probabilities)
fid_B = fopen(B_matrix_file, 'w');
% Print column headers (TF Names)
fprintf(fid_B, 'Peak_Coordinates');
for j = 1:length(Candidate_TFs)
    fprintf(fid_B, '\t%s', Candidate_TFs{j});
end
fprintf(fid_B, '\n');

% Print rows (Peaks as labels, followed by probabilities)
for i = 1:P
    % Format peak label as chr_start_end
    fprintf(fid_B, 'chr%d_%d_%d', Candidate_peaks.chr_num(i), Candidate_peaks.point1(i), Candidate_peaks.point2(i));
    
    % Extract row, convert to full dense array if sparse, and print
    row_data = full(Candidate_TF_Peak_Binding_prob(i, :));
    fprintf(fid_B, '\t%G', row_data);
    fprintf(fid_B, '\n');
end
fclose(fid_B);
fprintf('B matrix successfully written to: %s\n', B_matrix_file);

% 2. Write the L Matrix (Peak-Gene Looping Probabilities)
fid_L = fopen(L_matrix_file, 'w');
% Print column headers (Gene Symbols)
fprintf(fid_L, 'Peak_Coordinates');
for j = 1:G
    fprintf(fid_L, '\t%s', Candidate_genes.gene_symbols{j});
end
fprintf(fid_L, '\n');
...
fclose(fid_L);
fprintf('L matrix successfully written to: %s\n\n', L_matrix_file);