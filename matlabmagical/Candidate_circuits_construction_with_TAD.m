...
TF_num=full(sum(Candidate_TF_Peak_Binding)); % number of peaks per TF
TF_pct=full(sum(Candidate_TF_Peak_Binding))/length(Candidate_peaks.peak_index); % percentage of peaks per tf
...
TF_index=find(TF_pct>pct_threshold & TF_num>num_threshold & TF_enrichment_FC>enrichment_threshold);

disp(sprintf("Using thresholds:\n pct:%.2f\n peaks:%d\n enrichment:%.2f\n", pct_threshold, num_threshold, enrichment_threshold));

disp(sprintf("TFs passing cutoffs: %d", length(TF_index)));
disp(sprintf("TFs over 30 peaks: %d", sum(TF_num>30)));
disp(sprintf("TFs over 0.8 enrichment: %d\n", sum(TF_enrichment_FC>0.8)));
for thresh = 0.05:-0.01:0.01
    disp(sprintf("TFs over %.2f presence: %d", thresh, sum(TF_pct > thresh)));
end
...
fprintf('\nMAGICAL initially select %d TFs, %d peaks and %d genes for circuit inference\n\n', length(Candidate_TFs), length(Candidate_peaks.peak_index), length(Candidate_genes.gene_symbols))
disp(Candidate_genes.gene_symbols);
